...
document.querySelector('#filter-all').addEventListener('click', (e)=>{
    itemsEl.innerHTML = ''
    obtenerResultados('Controller?ACTION=PRODUCTO.FIND_ALL')
})
document.querySelector('#filter-coffee').addEventListener('click', (e)=>{
    itemsEl.innerHTML = ''
    obtenerResultados('Controller?ACTION=PRODUCTO.FILTER&FILTRO=Coffee')
})
document.querySelector('#filter-tea').addEventListener('click', (e)=>{
    itemsEl.innerHTML = ''
    obtenerResultados('Controller?ACTION=PRODUCTO.FILTER&FILTRO=Tea')
    
})

document.querySelector('#filter-price').addEventListener('click', (e)=>{
    itemsEl.innerHTML = ''
    obtenerResultados('Controller?ACTION=PRODUCTO.PRICE1') 
})

document.querySelector('#filter-price2').addEventListener('click', (e)=>{
    itemsEl.innerHTML = ''
    obtenerResultados('Controller?ACTION=PRODUCTO.PRICE2') 
})
...
/*async function fetchItemsCart() {
    cart_data = await obtenerResultados('Controller?ACTION=PRODUCTO.FILTER&FILTRO=Coffee');
}
fetchItemsCart();*/
...
/*function renderCoffeeItems() {
  filteredItems = ITEMS.filter(item => item.tipo === 'Coffee'); // Filtrar elementos por tipo 'Coffee'
  itemsEl.innerHTML = ''
...
function renderTeaItems() {
  filteredItems = ITEMS.filter(item => item.tipo === 'Tea'); // Filtrar elementos por tipo 'Tea'
  itemsEl.innerHTML = ''
  filteredItems.forEach((item, idx) => {
    if (item.tipo === 'Tea') {
      const itemEl = document.createElement('div');
...
function renderByPriceHigherItems() {
  filteredItems = ITEMS.sort((a, b) => b.precio - a.precio); // Ordenar elementos por precio descendente
  itemsEl.innerHTML = '';
  itemsEl.innerHTML = ''
  filteredItems.sort((a, b) => b.precio - a.precio).forEach((item, idx) => {
    const itemEl = document.createElement('div');
    itemEl.classList.add('item');
    itemEl.id = `item-${item.id}`;
    itemEl.onclick = () => addItem(idx, item.id);
...
function renderByPriceLowerItems() {
  filteredItems = ITEMS.sort((a, b) => a.precio - b.precio); // Ordenar elementos por precio ascendente
  itemsEl.innerHTML = ''
  filteredItems.sort((a, b) => a.precio - b.precio).forEach((item, idx) => {
    const itemEl = document.createElement('div');
    itemEl.classList.add('item');
    itemEl.id = `item-${item.id}`;
    itemEl.onclick = () => addItem(idx, item.id);
...
////////////////////
async function addItem(idx, itemId) {
    
  const item = ITEMS[itemId-1];
  const selectedItem = cart_data.find((item) => item.id.toString() === itemId.toString());
...
function addItem(idx, itemId) {
    ...
    const selectedItem = ITEMS.find((item) => item.id.toString() === itemId.toString());
}
}*/
